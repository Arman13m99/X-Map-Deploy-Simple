...
def load_polygon_data():
    """Load polygon data at startup"""
    global gdf_marketing_areas, gdf_tehran_region, gdf_tehran_main_districts
    global df_coverage_targets, target_lookup_dict
        ...
        try:
            if os.path.exists(file_path):
                ...
                if 'WKT' in df_poly.columns:
                    ...
                    logger.info(f"Loaded marketing areas for {city_name_key}: {len(gdf)} polygons")
        except Exception as e:
            logger.error(f"Error loading marketing areas for {city_name_key}: {e}")
            gdf_marketing_areas[city_name_key] = gpd.GeoDataFrame()
    
    # Load Tehran district shapefiles
    ...
    coverage_target_file = os.path.join('src', 'targets', 'tehran_coverage.csv')
    try:
        if os.path.exists(coverage_target_file):
            ...
            if 'marketing_area' in df_temp_targets.columns:
                ...
                # Drop unmapped areas
                unmapped_count = df_temp_targets['area_id'].isna().sum()
                if unmapped_count > 0:
                    logger.warning(f"Could not map {unmapped_count} areas from coverage targets")
                
                df_temp_targets = df_temp_targets.dropna(subset=['area_id'])
...
def get_initial_data():
    ...
    try:
        ...
        sample_orders = db_manager.get_orders(city_name="tehran").head(1000)  # Sample for performance
...
def get_map_data():
    ...
    try:
        ...
        df_orders_all_for_city = db_manager.get_orders(city_name=city_name)
...
def generate_population_heatmap(area_type_display: str, selected_polygon_sub_types: List[str], zoom_level: int) -> List[Dict]:
    ...
    if gdf_pop_source is None or 'Pop' not in gdf_pop_source.columns:
        return []
